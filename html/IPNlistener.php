...
if ($verified) {
    ...
    error_log($date. "Verified\n", 3, "/var/www/html/log.txt");
    error_log(print_r($_POST,true),3,"/var/www/html/log.txt");
    $db =ierg4210_DB();
    ...
    for ($i=1; $i<=$totalitem;$i++){
        ...
        error_log("before enter function : \n", 3, "/var/www/html/log.txt");
        $item_pid = ierg4210_getpid($item_name);
        $list .="{$item_pid}:{$item_quantity}&price={$item_amount},";
    }
    error_log("list initialized: \n" . $item_pid, 3, "/var/www/html/log.txt");
    ...
    error_log("Variable initialized \n", 3, "/var/www/html/log.txt");
