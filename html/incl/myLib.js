...
(function(){
	...
	myLib.processJSON = function(url, param, successCallback, opt) {
		opt = opt || {};
		opt.url = url || 'lib/admin-process.php';
	...
	myLib.authcart = function(param, successCallback) {
        param = param || {};
        param.rnd =  new Date().getTime(); // to avoid caching in IE
        myLib.processJSON('lib/cart-process.php?' + encodeParam(param), null, successCallback);
    };
	...
	myLib.auth =function(param, successCallback) {
        param = param || {};
        param.rnd =  new Date().getTime(); // to avoid caching in IE
        myLib.processJSON('lib/auth-process.php?' + encodeParam(param), null, successCallback);
    };
	...
	myLib.submit = function(form, successCallback) {
		myLib.validate(form) && myLib.ajax({
			method: 'POST',
			url: form.getAttribute('action') || 'lib/admin-process.php',
